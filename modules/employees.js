...
export function renderEmployeeTable({ getEmployees, getSearchQuery, getDepartmentFilter, getShowTerminated }) {
  ...
  const statusDot = `<span class="status-dot ${colorClass}" aria-label="Status: ${label}" title="${label}" style="background:${inlineColor};display:inline-block;width:10px;height:10px;border-radius:50%;vertical-align:middle;"></span>`;
    ...
    <td data-label=\"Name\" class=\"col-name cell-name px-2 py-2 font-semibold text-indigo-600 hover:text-indigo-700 cursor-pointer\" onclick=\"viewEmployee('${employee.id}', 'employees')\">${nameTxt}</td>
